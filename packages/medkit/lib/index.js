...
const waitForPushed = (page, re, timeout = 0) =>
  new Promise((resolve, reject) => {
    // Workaround
...
const setDataToClipboard = (page, type, data) =>
  new Promise(async (resolve, reject) => {
    await page.evaluate(
      (t, d) => {
        const onCopy = e => {
...
const selectRect = (page, selector) =>
  new Promise(async (resolve, reject) => {
...
const updatePost = (client, postId, html) =>
  new Promise(async (resolve, reject) => {
    const page = await newPage(client);
    await setDataToClipboard(page, "text/html", html);
    await page.goto(`https://medium.com/p/${postId}/edit`);
...
const destroyPost = (client, postId) =>
  new Promise(async (resolve, reject) => {
    const page = await newPage(client);
    await page.goto(`https://medium.com/p/${postId}/edit`);
    {
...
const close = client => client.browser.close();
